...
- **Rooms** (`/rooms`): Represents physical spaces on campus, tracking maximum capacity. Supports CRUD for Room objects (id, name, capacity).
- **Sensors** (`/sensors`): Definitions of physical hardware (e.g., CO2 monitors) assigned to Rooms. Supports CRUD for Sensor objects (id, type, status, currentValue, roomId).
- **Sensor Readings** (`/sensors/{sensorId}/readings`): Historical event logs for metric captures. Sub-resource for SensorReading objects (id, timestamp, value). POST updates the parent sensor's `currentValue`.
